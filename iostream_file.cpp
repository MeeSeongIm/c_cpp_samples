...
string lineno () {				// line number function
	static int lineint;
	static char linestr[3];
	if(lineint >= 99) lineint = 0;
	sprintf(linestr, "%02d", ++lineint);  // sprintf formatted 2-digit number
...
int main( int argc, char ** argv ) {
	...
	ofstream ofile(filename);				// out file stream
	...
	fstream afile(filename, fstream::in | fstream::out | fstream::app ); // input, output, append
	if(!afile.good()) {
